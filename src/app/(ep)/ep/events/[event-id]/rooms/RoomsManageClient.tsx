...
export default function RoomsManageClient({ eventId, initialRooms }: Props) {
  ...
  function handleSaveRoom() {
    ...
    startTransition(async () => {
      ...
      if (editingRoomId) {
        result = await updateEventRoom(editingRoomId, eventId, data)
      } else {
        result = await createEventRoom(eventId, data)
      }
      if ('error' in result) {
  ...
  function handleDeleteRoom(roomId: string) {
    setRoomError('')
    startTransition(async () => {
      const result = await deleteEventRoom(roomId, eventId)
      if ('error' in result) {
        setRoomError(result.error)
      } else {
        setConfirmingDeleteRoomId(null)
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <CSVImportPanel
            ...
            onImport={csvText => importEventRoomCSV(eventId, csvText)}
