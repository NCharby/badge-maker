-- ============================================================
-- Fix ambiguous "room_id" column reference in both Roommate
-- Finder RPCs.
--
-- Root cause: RETURNS TABLE (room_id UUID, ...) creates an
-- implicit PL/pgSQL OUT variable named "room_id". Unqualified
-- references to "room_id" in the room_leads and
-- confirmed_occupants CTEs were ambiguous between that variable
-- and the CTE column. Fix: alias occupant_data as "od" and
...
BEGIN
  ...
  -- will match — correct behaviour for Basic Event Rooms.
