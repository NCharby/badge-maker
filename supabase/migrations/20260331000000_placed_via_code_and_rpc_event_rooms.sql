-- ============================================================
-- placed_via_code + fix RPCs to handle event-scoped rooms
--
-- 1. Add placed_via_code to event_attendees
--    Tracks whether a roommate was placed via Roommate Code (vs application/manual).
--
-- 2. Fix get_roommate_finder_cards and get_roommate_finder_cards_ep
--    Both RPCs previously bailed out when venue_id was NULL, making the
--    Basic Event Rooms module (rooms.event_id) completely invisible.
--    Fix: remove the early NULL bail; update WHERE to match both scopes.
-- ============================================================

-- ── 1. Add placed_via_code ────────────────────────────────────────────────────

ALTER TABLE public.event_attendees
  ADD COLUMN IF NOT EXISTS placed_via_code BOOLEAN NOT NULL DEFAULT false;
...
BEGIN
  ...
  -- NOTE: We no longer bail out when v_venue_id IS NULL.
  -- When v_venue_id IS NULL, the condition (r.venue_id = v_venue_id) is always
  -- false (NULL ≠ NULL in SQL), so only event-scoped rooms (r.event_id = p_event_id)
  -- will match — which is exactly the behaviour needed for Basic Event Rooms.
  ...
  -- KEY FIX: match venue-scoped rooms (r.venue_id = v_venue_id)
  -- OR event-scoped rooms (r.event_id = p_event_id)
...
BEGIN
  ...
  -- KEY FIX: match venue-scoped OR event-scoped rooms
  WHERE (r.venue_id = v_venue_id OR r.event_id = p_event_id)
